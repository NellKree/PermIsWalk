namespace Mapbox.Examples
{
	...
	public class QuadTreeCameraMovement : MonoBehaviour
	{
		...
		{
			var zoom = Mathf.Max(4.0f, Mathf.Min(_mapManager.Zoom + zoomFactor * _zoomSpeed, 18.0f));
		...
		void PanMapUsingKeyBoard(float xMove, float zMove)
		{
			if (Math.Abs(xMove) > 0.0f || Math.Abs(zMove) > 0.0f)
			{
                ...
                //	if ((topLeftBorderPosition.x >= -horzExtent && offsetX < 0) || (bottomRightBorderPosition.x <= horzExtent && offsetX > 0))
                //	{
                //		offsetX = 0;
                //	}

                //	if ((topLeftBorderPosition.z <= vertExtent && offsetZ > 0) || (bottomRightBorderPosition.z >= -vertExtent && offsetZ < 0))
                //	{
                //		offsetZ = 0;
                //	}
                //}

                //_velocity = new Vector3(offsetX, 0, offsetZ);

                //var latlongDelta = Conversions.MetersToLatLon(new Vector2d(offsetX * factor, offsetZ * factor));
